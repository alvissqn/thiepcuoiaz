...
function datePickerInit(thisEl){
		...
		switch(type){
			...
			case "hours":
				if(timeClass.length>0){
					...
					nHTML+='<div class="row ml-0 mr-0"><div data-show="time" class="form-date-nav primary-actived col-6"><i class="fa fa-clock-o"></i> '+msg["selectHour"]+'</div><div data-show="date" class="form-date-nav col-6"><i class="fa fa-calendar"></i> '+msg["selectDate"]+'</div></div>';
					tHTML+='<div class="form-date-warning"></div>';
					tHTML+='<select class="form-control" data-type="'+type+'">';
					...
					if(cog["allow"]["minutes"].length>0){
						tHTML+='<select class="form-control" data-type="minutes">';
			...
			//Tháng
			case "months":
				dHTML+='<select class="form-control" data-type="'+type+'">';
				for(var i=1; i<=12; i++){
					if(i>9){
						var val=i;
					}else{
			...
			case "min":
				...
				dHTML+='<select class="form-control" data-type="years">';
				var i=max["y"];
				for(i; i>=min["y"]; i--){
	...
	wrap.children(".form-date-picker").html('<div> '+nHTML+' <div class="form-date-body form-date-body-time">'+tHTML+'</div> <div class="form-date-body form-date-body-date '+timeClass+'">'+dHTML+'<table class="form-date-day w-100"></table></div> </div>').toggle();
	...
	wrap.on("click", ".primary-hover", function(){
		...
		wrap.find(".form-control").val(output);
		wrap.find(".form-control").change();
...
$(document).on('click', function(e){
	if( $(e.target).closest('.form-date-wrap').length == 0 ){
		if( $('.form-date-picker').is(':visible') ){
