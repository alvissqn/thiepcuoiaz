    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
	 ...
	 if (value != "") {
	   ...
	   // getting json data from file for search results
		var searchData = {listItems: []};
		$('#main-menu-navigation a').each(function(){
			searchData.listItems.push({
				name: $(this).children('.menu-title, .menu-item').text(),
				icon: $(this).children('i').attr('class'),
				url: $(this).attr('href')
			});
		});

		for (var i = 0; i < searchData.listItems.length; i++) {
		  ...
		  if (
		    searchData.listItems[i].name.toLowerCase().indexOf(value) == 0 &&
		    a < 10 || !(searchData.listItems[i].name.toLowerCase().indexOf(value) == 0) &&
		    searchData.listItems[i].name.toLowerCase().indexOf(value) > -1 &&
		    a < 10
		  ) {
		    if (a === 0) {
			 $activeItemClass = "current_item"
		    ...
		    $startList +=
			 ...
			 '">' +
			 '<a class="d-flex align-items-center justify-content-between w-100" href=' +
			 searchData.listItems[i].url +
			 ">" +
			 '<div class="d-flex justify-content-start">' +
			 '<span class="mr-75 ' +
			 searchData.listItems[i].icon +
			 '" data-icon="' +
			 searchData.listItems[i].icon +
			 '"></span>' +
			 "<span>" +
		...
		if ($startList == "" && $otherList == "") {
		  $otherList =
		    ...
		    "<span>"+__('admin/general.search_no_result')+"</span>" +
		    "</div>" +
		    "</a>" +
		    "</li>"
		}
